...
//
//   1 - .keystone_preinstall  (optional)
//   2 - .keystone_install     (required)
//   3 - .keystone_postinstall (optional)
//
// Scripts (1) and (3) will be run via the |runner| delegate, if they are
// present. Script (2) will not be run by the |runner|, rather the
// KSInstallAction itself will run the .keystone_install script.
//
// Environment variables
// ---------------------
// Keystone makes certain environment variables available to install scripts.
// Some of these variables are just informative data from Keystone, while others
// are there to facilitate IPC between the scripts.
//
...
// KS_PREINSTALL_OUT set to the stdout of the .keystone_preinstall script. 
// Similarly, scripts run after the install script will have the variable 
// KS_INSTALL_OUT set to the stdout of the .keystone_install script.
//
